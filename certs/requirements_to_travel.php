                    ...
                    <div class="body-text">
                        ...
                        <p class="body-letter">
                            This is to certify that <strong><?php echo $first_name . "&nbsp" . $middle_name . "&nbsp" . $last_name; ?></strong>, <?php $diff = date_diff(date_create($dob), date_create(date("Y-m-d")));
                                                                                                                                                    echo $diff->format('%y'); ?> years of age,
                            <input placeholder="Position and Company" class="cot-input" type="text" onkeypress="this.style.width = (this.value.length + 1) + 'em';">,
                            <input placeholder="Purpose" class="cot-input" type="text" onkeypress="this.style.width = (this.value.length + 1) + 'em';">, is subject to travel to
                            <input placeholder="Destination 1" class="cot-input" type="text" onkeypress="this.style.width = (this.value.length + 1) + 'em';">, going to
                            <input placeholder="Destination 2" class="cot-input" type="text" onkeypress="this.style.width = (this.value.length + 1) + 'em';"> on
                            <input type="date" class="input-4 cot-input">.
                        </p>
                        <br>
                        <p class="body-letter">
                            This certifies further that he is <strong>NOT</strong> Person Under Monitoring (PUM)/NOT Person Under Investigation (PUI) due to COVID Pandemic while in the AOR and <?php echo $address; ?> is <strong>Low Risk Barangay</strong>.
                        </p>
                        <p class="body-letter">
                            Issued this
                            <?php echo date("d"); ?>'th day of
                            <?php echo date("M, Y"); ?> prior to his travel from this place to other Municipality and for any other purpose this may serve.
                        </p>
                    </div>
                </div>
